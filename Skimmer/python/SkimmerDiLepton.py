...
class SkimmerDiLepton(Module):
	...
	def fillJetBranches(self, event, jetSyst):
		...
		for i, jet in enumerate(event.jetsSel):
			...
			jer_from_pt = self.getJetPtResolution(getattr(jet,jetPtName), jet.eta, rho)
			jer_from_pt_nom = self.getJetPtResolution(getattr(jet, "pt_nom") if self.isMC and hasattr(jet, "pt_nom") else jet.pt, jet.eta, rho)
			jer_from_pt_nano = self.getJetPtResolution(jet.pt, jet.eta, rho)
